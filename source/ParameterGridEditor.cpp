    ...
    , warning (newWarning)
{
    ...
    if (ceilingfangProcessor != nullptr)
    {
        ...
        {
            ceilingfangProcessor->setAuditionEnabled (! ceilingfangProcessor->isAuditionEnabled());
        ...
        {
            ceilingfangProcessor->setAuditionType (1 - ceilingfangProcessor->getAuditionType());
            syncAuditionControls();
        };
...
void ParameterGridEditor::timerCallback()
{
    ...
    {
        const auto latestInputPeak = ceilingfangProcessor->getInputPeakLevel();
        const auto latestPeak = ceilingfangProcessor->getOutputPeakLevel();
...
{
#if defined(YUP_AUDIO_PLUGIN_ENABLE_STANDALONE)
    if (ceilingfangProcessor == nullptr || auditionButton == nullptr || auditionTypeButton == nullptr)
        return;

    auditionButton->setButtonText (ceilingfangProcessor->isAuditionEnabled() ? "Audition On" : "Audition Off");
    auditionTypeButton->setButtonText (ceilingfangProcessor->getAuditionType() == 0 ? "Saw" : "Pulse");
    static_cast<ehl::ui::CommandButton*> (auditionButton.get())->setSelected (ceilingfangProcessor->isAuditionEnabled());
    static_cast<ehl::ui::CommandButton*> (auditionTypeButton.get())->setSelected (ceilingfangProcessor->getAuditionType() != 0);
#endif
}
